...
def parse_pdfs():
    ...
    grok_api_key = data.get('grok_api_key', None)
    ...
    if not os.path.isdir(folder_path):
        return jsonify({'error': '请提供文件夹路径，不是文件路径'}), 400
    ...
    if not pdf_files:
        return jsonify({'error': '文件夹中没有找到PDF文件'}), 400
    
    # 解析PDF文件
    results, success_count, error_count = process_pdf_files(
        pdf_files, use_ocr, grok_api_key
    )
